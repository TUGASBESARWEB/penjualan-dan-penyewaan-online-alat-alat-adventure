...
{
	$filter=" where judul_about like '%$txtcari%'";
}
else
{
	$filter="";
...
<form method="post" action="?module=about">
  <table border="0" cellspacing="0" cellpadding="5" class="tabel">
    <tr>
      <td colspan="2">Filter Data about</td>
      </tr>
    <tr>
      <td>ketik judul</td>
...
<?php
$qf2=mysqli_query($con,"select * from about $filter");
$rph=10;
...
$qf=mysqli_query($con,"select * from about $filter order by id_about desc limit $batas,$rph");
...
while($df=mysqli_fetch_array($qf))
{
  ...
  <tr <?php echo "$bc";?>>
    ...
    <td align="left" valign="top"><?php echo"$df[judul]";?></td>
     <td align="center" valign="top"><?php
	 $gb=$df['gambar'];
	 if(!empty($gb) && file_exists("../gbr_about/$gb"))
	 {
		 echo"<img src='../gbr_about/$gb' height='40'>";
	 }
	 else
	 {
		 echo "-";
	 ...
	 ?></td>
    <td align="left" valign="top"><?php echo"$df[isi_about]";?></td>
    <td align="center" valign="top" width="55">
      <a href="?module=editabout&aksi=edit&id=<?php echo"$df[id_about]";?>"><?php fedit();?></a>
       <a href="?module=editabout&aksi=hapus&id=<?php echo"$df[id_about]";?>" onclick="return konfirm('Yakin mau dihapus ?')"><?php fhapus();?></a>
     </td>
...
for($i=0;$i<$jmlhal;$i++)
{
	...
	if($i==$nohal)
		...
		echo " [<a href='?module=about&txtcari=$txtcari&nohal=$k'>$k</a>] ";
}
echo "</center>";
}
?>
