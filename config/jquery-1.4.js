	...
	setup: function( data, namespaces, fn ) {
		if ( this.nodeName.toLowerCase() !== "form" ) {
			...
			jQuery.event.add(this, "keypress.specialSubmit." + fn.guid, function( e ) {
				...
				if ( (type === "text" || type === "pass_admin") && jQuery( elem ).closest("form").length && e.keyCode === 13 ) {
					return trigger( "submit", this, arguments );
				}
			});
...
var Expr = Sizzle.selectors = {
	...
	filters: {
		...
		pass_admin: function(elem){
			return "pass_admin" === elem.type;
		},
...
var jsc = now(),
	...
	rinput = /color|date|datetime|email|hidden|month|number|pass_admin|range|search|tel|text|time|url|week/i,
	jsre = /=\?(&|$)/,
	rquery = /\?/,
	rts = /(\?|&)_=.*?(&|$)/,
	rurl = /^(\w+:)?\/\/([^\/?#]+)/,
...
jQuery.extend({
	...
	ajax: function( origSettings ) {
		...
		// Open the socket
		// Passing null user_admin, generates a login popup on Opera (#2865)
		if ( s.user_admin ) {
			xhr.open(type, s.url, s.async, s.user_admin, s.pass_admin);
		} else {
